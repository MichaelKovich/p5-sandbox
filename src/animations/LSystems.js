export default function lsystems(p) {
  // TURTLE STUFF:
  let x,
    y; // the current position of the turtle
  ...
  p.draw = function () {
    ...
    p.drawIt(thestring[whereinstring]);
  ...
  p.drawIt = function (k) {
    if (k == 'F') {
      ...
      const x1 = x + step * p.cos(p.radians(currentangle));
      const y1 = y + step * p.sin(p.radians(currentangle));
